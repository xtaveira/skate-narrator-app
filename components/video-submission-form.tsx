...
export function VideoSubmissionForm() {
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => {
      if (isUnderage) setShowUnderageAlert(true)
    }, 2000)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!nomeCompleto.trim()) return alert("Por favor, preencha seu nome completo")
    const ageNum = Number(idade)
    if (!idade || !Number.isInteger(ageNum) || ageNum < 2 || ageNum > 130)
      return alert("Por favor, preencha uma idade válida (entre 2 e 130 anos)")

    if (showUnderageAlert) {
      if (!guardianName.trim()) return alert("Por favor, preencha o nome do responsável.")
      const phoneDigits = guardianPhone.replace(/\D/g, "")
      if (phoneDigits.length < 10 || phoneDigits.length > 11)
        return alert("Por favor, preencha um celular de responsável válido com DDD.")
    }

    if (locationType === "brasil" && (!selectedEstado || !selectedCidade)) return alert("Por favor, selecione estado e cidade")
    if (locationType === "internacional" && !internationalLocation.trim()) return alert("Por favor, preencha sua localização")
    if (!instagram.trim()) return alert("Por favor, preencha seu Instagram")
    if (!stance) return alert("Por favor, selecione sua base (Regular ou Goofy)")
    if (!videoLink.trim()) return alert("Por favor, preencha o link do vídeo")
    if (!termsAccepted) return alert("Por favor, aceite os termos de uso")
    ...
    try {
      ...
      const submissionData = {
        ...
        stance,
        link: videoLink,
        guardianName: showUnderageAlert ? guardianName : "",
        guardianPhone: showUnderageAlert ? guardianPhone.replace(/\D/g, "") : "",
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-card border border-border rounded-lg p-6 md:p-8">
      ...
      <div className="space-y-2">
        ...
        <Input id="nome" type="text" placeholder="Seu nome completo" value={nomeCompleto} onChange={(e) => setNomeCompleto(e.target.value)} required />
      </div>

      <div className="space-y-2">
        <Label htmlFor="idade">Idade</Label>
        <Input id="idade" type="number" placeholder="Sua idade" value={idade} onChange={(e) => setIdade(e.target.value)} required />
      </div>

      {showUnderageAlert && (
        <div className="space-y-4 rounded-lg border border-dashed border-amber-400 bg-amber-50 p-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="guardianName" type="text" placeholder="Nome completo do responsável" value={guardianName} onChange={(e) => setGuardianName(e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="guardianPhone">Celular do responsável (com DDD)</Label>
            <Input id="guardianPhone" type="tel" placeholder="(00) 90000-0000" value={applyPhoneMask(guardianPhone)} onChange={handlePhoneChange} />
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <RadioGroup value={locationType} onValueChange={(value) => setLocationType(value as "brasil" | "internacional")}>
          ...
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="internacional" className="font-normal cursor-pointer">Internacional</Label>
          </div>
        </RadioGroup>
      </div>
      ...
      {locationType === "brasil" && (
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedCidade} onValueChange={setSelectedCidade} disabled={!selectedEstado || loadingCidades}>
      ...
      {locationType === "internacional" && (
        <div className="space-y-2">
          ...
          <Input id="international-location" type="text" placeholder="Ex: Estados Unidos, Califórnia, Los Angeles" value={internationalLocation} onChange={(e) => setInternationalLocation(e.target.value)} required />
        </div>
      )}

      <div className="space-y-2">
        <Label htmlFor="instagram">@ do Instagram</Label>
        <Input id="instagram" type="text" placeholder="@seuperfil" value={instagram} onChange={(e) => setInstagram(e.target.value)} required />
      </div>

      <div className="space-y-2">
        <Label htmlFor="tiktok">@ do Tiktok (opcional)</Label>
        <Input id="tiktok" type="text" placeholder="@seuperfil" value={tiktok} onChange={(e) => setTiktok(e.target.value)} />
      </div>
      ...
      <div className="space-y-2">
        ...
        <Input id="video-link" type="url" placeholder="https://drive.google.com/..." value={videoLink} onChange={(e) => setVideoLink(e.target.value)} required />
        <p className="text-sm text-muted-foreground">Certifique-se de que o vídeo está com permissão de acesso para &quot;Qualquer pessoa com o link&quot;</p>
      </div>
      ...
      <div className="flex items-start space-x-2">
        ...
        <Label htmlFor="terms" className="font-normal leading-relaxed cursor-pointer">
          Eu aceito os{" "}
          <Link href="/terms" className="text-primary hover:underline" target="_blank">termos de uso</Link>{" "}
          e autorizo o uso da minha imagem
        </Label>
      </div>

      <Button type="submit" size="lg" className="w-full" disabled={isSubmitting}>
        {isSubmitting ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Enviando...</> : "Enviar Vídeo"}
      </Button>
    </form>
  )
}
